...
void init_index_and_register(Table *table);
...
Table *table_create(char *path, char ***raw_data, char *format, int num_rows)
{
    ...
    // Inicializa os valores de indice e registro atual da tabela
    init_index_and_register(table);
    // Coloca o estado da busca como 0
...
// Inicializa alguns valores da tabela que são mais extensos
// e são apenas armazenados em runtime
void init_index_and_register(Table *table)
{
    Register reg;
    reg.byte_offset = table_header_size;
    reg.tam_reg = 0;
    reg.prox_reg = -1;
    reg.data = NULL;
    reg.removed = '1';
...
{
    if (table->current_register.data != NULL)
        free(table->current_register.data);

    fseek(table->f_pointer, table_header_size, SEEK_SET);
    table->current_register.byte_offset = table_header_size;
    table->current_register.tam_reg = 0;
    table->current_register.prox_reg = -1;
    table->current_register.data = NULL;
...
bool table_move_to_next_register(Table *table)
{
    ...
    register_free(&(table->current_register));
    // lê o registro indicado no next_register_offset
    table->current_register = register_read(next_register_offset, table->f_pointer);
...
bool table_delete_current_register(Table *table)
{
    ...
    {
        before = register_read(table->top, table->f_pointer);
        ...
        next = register_read(before.prox_reg, table->f_pointer);
        ...
        while (next.prox_reg > 0)
        {
            ...
            register_free(&before);
            before = next;
            next = register_read(next.prox_reg, table->f_pointer);
        }
...
inserted:
    ...
    if (before.byte_offset != -1)
    {
        register_write(before, table->f_pointer, table->format);
        register_free(&before);
    }

    if (next.byte_offset != -1)
    {
        register_write(next, table->f_pointer, table->format);
        register_free(&next);
    }

    register_write(table->current_register, table->f_pointer, table->format);
...
bool table_insert_new_row(Table *table, char **row)
{
    ...
    else
    {
        ...
        register_write(new_register, table->f_pointer, table->format);
        table->num_removed -= 1;
        table->num_reg += 1;
    }
...
bool table_create_index(Table *table, char *path, int key_row, int key_size)
{
    ...
    while (table_move_to_next_register(table))
    {
        void *_d = get_data_in_collumn(table->current_register.data, table->format, key_row);
        fwrite(_d, sizeof(char), key_size, file);
...
bool table_load_index(Table *table, char *path, int key_row, int key_size)
{
    ...
    table->index.num_reg = table->num_reg;
